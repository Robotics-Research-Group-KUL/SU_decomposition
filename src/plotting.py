...
def plot_frame(ax, T, color="b", linewidth=3.0, arrow_len=0.08):
    p = T[:3, 3]
    R = T[:3, :3]
    for k in range(3):
        ax.quiver(
...
def plot_frames(ax, T, key_values, color="b", linewidth=3.0, arrow_len=0.08):
    for j in key_values:
    ...
    for row_subplot in range(nb_rows):
        for col_subplot in range(nb_columns):
            ...
            if row_subplot < 0.5:
                ...
                # ax.set_yticks([-2, 0, 2])
                ax.set_title(
                    rf"$\omega_{{{axis_labels[col_subplot]}}}~[{{{y_axis_units[0]}}}]$",
                ...
                ax.set_title(
                    rf"$v_{{{axis_labels[col_subplot]}}}~[{{{y_axis_units[1]}}}]$",
                    fontsize=fontsize_title,
                )
...
def plot_twist_trajectory(axes, twist_trajectory, s_max, color="b"):
    ...
    for row_subplot in range(nb_rows):
        for col_subplot in range(nb_columns):
            ...
            ax.plot(
                progress_axis,
                twist_trajectory[row_subplot * nb_columns + col_subplot, :],
                color=color,
            )
            ax_counter += 1
    ...
    for row_subplot in range(nb_rows):
        for col_subplot in range(nb_columns):
            ...
            if row_subplot < 0.5:
                ...
                # ax.set_yticks([-2, 0, 2])
                ax.set_title(
                    rf"$f_{{{axis_labels[col_subplot]}}}~[{{{y_axis_units[0]}}}]$",
                    fontsize=fontsize_title,
                )
            else:
                # ax.set_ylim([-0.5, 0.5])
                # ax.set_yticks([-0.5, 0, 0.5])
                ax.set_title(
                    rf"$\tau_{{{axis_labels[col_subplot]}}}~[{{{y_axis_units[1]}}}]$",
                    fontsize=fontsize_title,
                )
    ...
    for row_subplot in range(nb_rows):
        for col_subplot in range(nb_columns):
            ...
            if row_subplot < 2.5:
                ...
                # ax.set_yticks([-2, 0, 2])
                ax.set_title(
                    rf"${{{name_component}}}_{{{idx}}}~[{{{y_axis_units[0]}}}]$",
                    fontsize=fontsize_title,
                )
            else:
                # ax.set_ylim([-0.5, 0.5])
                # ax.set_yticks([-0.5, 0, 0.5])
                ax.set_title(
                    rf"${{{name_component}}}_{{{idx}}}~[{{{y_axis_units[1]}}}]$",
                    fontsize=fontsize_title,
                )
