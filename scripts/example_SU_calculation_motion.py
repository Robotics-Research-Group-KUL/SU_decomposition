...
# options: 'translation_3D', 'rotation_1D', 'rotation_3D', 'pouring', 'contour_following',
...
T_raw, N, dt = _dh.load_demo_trajectory_motion(input_trajectory, path_to_data)
...
if progress_domain == "time":
    ...
    s = _pp.calculate_geom_progress_axis(T_raw, dt, L=0.3)
    ds = 0.02  # -> 2 cm
    T = _pp.preprocess_pose_data(T_raw, s, ds)
    N = T.shape[2]
...
if input_trajectory == "pouring":
    object_data = _dh.load_data_kettle(path_to_data)
    T_kettle_wrt_tracker = _dh.load_tracker_kettle_calibration_data()
    ...
    object_data = _dh.create_cube_data()
...
colors = ["r", "b"]
for j in range(nb_body_frame_transformations):
    ax = _plotting.plot_trajectory_origin(ax, T_var[j], color=colors[j], linewidth=3.0)
    ax = _plotting.plot_frames(
        ax,
        T_var[j],
...
fig_twist, axes_twist = _plotting.initialize_plot_twist_trajectory(progress_domain, input_trajectory)
fig_dutir, axes_dutir = _plotting.initialize_plot_dutir(progress_domain, input_trajectory)
fig_dutir_reg, axes_dutir_reg = _plotting.initialize_plot_dutir(progress_domain, input_trajectory)
